        ...
        if mode == 1:
            ...
            if(over_limit == 1):
				...
				# ~ if x == 0 and y == 0 and z == 0:
					# ~ dt = 0.2
					#set_inverse_client(round(data_x,4), round(data_y,4), round(data_z,4), 0 ,pitch, 0, grip_joint, 0.25)

				# ~ else:						
				if((pos_x < prev_x-0.001 or pos_x > prev_x+0.001) or (pos_y < prev_y-0.001 or pos_y > prev_y+0.001) or (pos_z < prev_z-0.001 or pos_z > prev_z+0.001)):
					print(pos_x,pos_y,pos_z)
					response = set_inverse_client(pos_x, pos_y, pos_z, yaw ,pitch, roll, grip_joint, dt)
					prev_x = pos_x
					prev_y = pos_y
					prev_z = pos_z
					prev_j1 = joint1
